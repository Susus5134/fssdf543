...
const socksAgent = new SocksProxyAgent('socks5://warp:good.360.cn@127.0.0.1:40000'); // 替换为您的 socks5 代理地址
...
class Bums {
    ...
    async tap(token) {
        try {
            ...
            if (actionResponse.msg == 'OK') {
                ...
                this.log(`Error while performing tap: ${actionResponse.msg}`, 'error');
            }

        } catch (error) {
            this.log(`Error while performing tap: ${error.message}`, 'error');
        }
    }
    ...
    async finishTask(token, taskId) {
        try {
            ...
            this.log(`Error completing task: ${error.message}`, 'error');
            return {msg: 'ERROR'}
        }
    }
        ...
        try {
            ...
            tasks.forEach(async task => {
                ...
                if (finishTask.msg == 'OK') {
                    ...
                    this.log(`Error completing task: ${finishTask.msg}`, 'error');
                }

                await this.countdown(Math.floor(20));
            });
        } catch (error) {
            this.log(`Error while performing task: ${error.message}`, 'error');
        }
    }
                ...
                for (const mine of mines.data.lists) {
                    while ((money - mine.nextLevelCost) > 0 && mine.limitText == '') {
                        try {
                            ...
                            this.log(`Perform mine upgrade ${mine.mineId} level up ${level + 1}`, 'info');
                            const upgradeResponse = await this.upgradeMine(token, mine.mineId);
                            if (upgradeResponse.msg == 'OK') {
                                money -= mine.nextLevelCost;
                                this.log(`Mine upgraded ${mine.mineId} level up ${level + 1}`, 'success');
                            } else {
        ...
        try {
            ...
            if (signLists.msg == 'OK' && signLists.data.signStatus == 0) {
                ...
                if (response.data.msg == 'OK') {
                    this.log('Received daily sign reward', 'success');
                }
            }
        } catch (error) {
            this.log(`Error when performing daily sign: ${error.message}`, 'error');
        }
    }
            ...
            for (let no = 0; no < initDataList.length; no++) {
                ...
                try {
                    ...
                    if (authResponse.msg == 'OK') {
                        ...
                        this.log(`Error while verifying account ${no + 1}: ${authResponse.msg}`, 'error');
                    }
    ...
    async waitWithCountdown(seconds) {
        for (let i = seconds; i >= 0; i--) {
            ...
            process.stdout.write(`===== All accounts completed, waiting ${i} seconds to continue loop =====`);
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
        console.log('');
    }
